...
const schema = z
    .object({
        ...
        confirmPassword: z
            ...
            .min(
                1,
                t("auth.resetPassword.fields.confirmPassword.error.required"),
            ),
    })
...
async function onSubmit() {
    ...
    if (password.value !== confirmPassword.value) {
        toast.add({
            title: t("auth.resetPassword.errors.title"),
            description: t(
                "auth.resetPassword.fields.confirmPassword.error.match",
            ),
            color: "error",
        })
        return
    ...
    if (error) {
        toast.add({
            title: t("auth.resetPassword.errors.title"),
            description:
                error.message || t("auth.resetPassword.errors.invalid"),
            color: "error",
        })
    } else {
        toast.add({
            ...
            >
                ...
                >
                    <UInput
                        ...
                        type="password"
                        :placeholder="
                            t('auth.resetPassword.fields.password.placeholder')
                        "
                        size="lg"
                        :disabled="loading"
                    />
