...
onMounted(async () => {
    ...
    try {
        ...
        if (result) {
            ...
            errorMessage.value = error.value || 'Failed to authenticate with YouTube'
        }
    } catch (e: any) {
            ...
            <div class="bg-gray-800 rounded-2xl p-8 text-center">
                ...
                <div v-if="processing" class="py-8">
                    <div class="w-16 h-16 mx-auto mb-6 relative">
                        <div class="absolute inset-0 border-4 border-red-600/30 rounded-full"></div>
                        <div
                            class="absolute inset-0 border-4 border-red-600 rounded-full border-t-transparent animate-spin"
                        ></div>
                    </div>
                    <h2 class="text-xl font-semibold text-white mb-2">Connecting to YouTube...</h2>
                    <p class="text-gray-400">Please wait while we complete the authentication</p>
                </div>
                ...
                <div v-else-if="success" class="py-8">
                    ...
                    <h2 class="text-xl font-semibold text-white mb-2">Connected to YouTube!</h2>
                    <p class="text-gray-400">Redirecting to your video library...</p>
                </div>
